...
export default function Popup() {
  ...
  const startRecording = async () => {
    try {
      ...
      setWebCamStatus(true); // Trigger re-render with webcam enabled
  ...
  return (
    <div>
      <div className="video-preview top-[56%] fixed ">
        {webCamStatus && isAuthenticated ? (
          <video
            ref={webcamPreviewRef}
            className="border border-gray-300 rounded-full w-64 h-64 object-cover"
            autoPlay
            muted
          />
        ) : !webCamStatus && isAuthenticated ? (
          <img
            src={authDetails?.image}
            alt="User profile"
            className="border border-gray-300 rounded-full w-64 h-64 object-cover"
          />
        ) : (
          <img
            src="https://ui-avatars.com/api/?name=H+p&size=320&rounded=true"
            alt="Default avatar"
            className="border border-gray-300 rounded-full w-64 h-64 object-cover"
          />
        )}
      ...
      <div
        ...
        className={`w-[300px] rounded-3xl bg-white fixed shadow-xl px-6 pt-2 pb-5 mx-6 my-5 z-[9999999]`}
